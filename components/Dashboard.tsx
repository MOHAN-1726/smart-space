...
// ------ SHARED STYLES ------
const inputClasses = "block w-full p-2 border rounded-lg bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-offset-slate-900 transition-all duration-200";
const searchInputClasses = "pl-10 pr-4 py-2 w-full border rounded-lg bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-offset-slate-900 transition-all duration-200";
...
const DashboardExamCard: React.FC<{ exam: Exam, user: User }> = ({ exam, user }) => {
    ...
    return (
        <div className="p-4 rounded-xl bg-gradient-to-r from-slate-50 to-blue-50/50 dark:from-slate-800/50 dark:to-primary-900/20 border border-slate-200/50 dark:border-slate-700/50 flex justify-between items-center hover-lift">
            <div>
...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout, theme, toggleTheme }) => {
    ...
    return (
        <div className="flex h-screen bg-slate-100 dark:bg-slate-900 overflow-hidden">
            {/* Sidebar */}
            <div className={`fixed inset-y-0 left-0 z-30 w-64 glass-card shadow-2xl transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 lg:flex-shrink-0 flex flex-col border-r border-slate-200/50 dark:border-slate-700/50`}>
                <div className="flex items-center justify-between p-4 border-b border-slate-200/50 dark:border-slate-700/50 flex-shrink-0">
                    <h1 className="text-xl font-bold gradient-text">Student Portal</h1>
                ...
                <nav className="flex-1 mt-4 p-2 space-y-1 overflow-y-auto">
                    {menuItems.map(item => (
                        <a key={item.name} href="#" onClick={(e) => { e.preventDefault(); setActiveView(item.view); setIsSidebarOpen(false); }}
                            className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${activeView === item.view ? 'bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-lg shadow-primary-500/30' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50'}`}>
            ...
            <div className="flex-1 flex flex-col w-full overflow-hidden">
                {/* Header */}
                <header className="flex-shrink-0 glass-card shadow-lg z-10 border-b border-slate-200/50 dark:border-slate-700/50">
                    <div className="flex items-center justify-between p-4">
                        ...
                        <div className="flex items-center gap-2 sm:gap-4">
                            {['exams'].includes(activeView) && (
                                <select value={selectedClassId || ''} onChange={e => setSelectedClassId(e.target.value)} className="p-2 border rounded-lg bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm max-w-[150px] sm:max-w-xs transition-all duration-200">
